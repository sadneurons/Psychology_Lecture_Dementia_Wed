...
A talk about where dementia care is right now — what's changed, what's genuinely new, and what we can all do about it. Questions very welcome at the end.

:::

## What we'll cover tonight :watch:
- **What dementia actually is** — and why "Alzheimer's" isn't the whole story
- **Getting the diagnosis right** — the different types, and the treatable mimics
- **The treatments we already have** — what they do, and what they honestly don't
- **The new disease-modifying drugs** — the first that touch the disease itself
- **Hope, and what we can do** — prevention, brain health, and joining the research

*Plenty of time for your questions at the end.*
...
::: {.incremental}
**Dementia** is an umbrella term. It describes a point where someone can no longer manage daily life on their own, because of problems with memory and thinking.

It is not one disease. There are many different causes — and each is defined in a different way:

1. Alzheimer's disease `defined by the proteins that build up in the brain`
2. Vascular dementia `defined by its cause — the brain's blood supply`
3. Dementia with Lewy bodies `defined by a particular protein deposit`
4. Frontotemporal dementia `defined by where in the brain it strikes`
:::
...
## Alzheimer's is a disease, like any other disease

- Alzheimer's is defined by two proteins building up in the brain: **amyloid** (which forms plaques) and **tau** (which forms tangles).

- It's the brain changes that make it Alzheimer's — not just the symptoms. Someone can have dementia *without* these proteins, and a few people have the proteins *without* any symptoms at all.

- That distinction matters, because it's now something we can actually measure in a living person.
...
## How we define Alzheimer's has completely changed {.small}
...
- Diagnosis from symptoms alone → "probable Alzheimer's"
...
- Which means we can spot it *before* symptoms appear
:::
...
*The same four ingredients clinicians use — in plain terms. Click each tab.*
...
::: {.column width="55%"}
**The plaque protein** — the earliest sign.
...
![click to reveal: a lumbar puncture — not as bad as people fear](Pictures/BHC_LP.jpeg)
:::
:::
...
::: {.column width="55%"}
**The tangle protein** — tracks more closely with symptoms.

- A spinal-fluid test
- **A blood test, "p-tau217"** — the breakthrough that makes testing in everyday clinics possible
...
::: {.column width="55%"}
**The wear-and-tear signal** — shows the brain is being injured, but not *what* is doing it.
...
::: {.column width="55%"}
**Everything else that adds up** — added to the picture in 2024.
...
## The number of people with dementia is rising fast {.smaller}
::: {.incremental}

- Alzheimer's is the most common cause of dementia in people over 65 — but it's far from the only one.

- In 2020, more than **55 million** people worldwide were living with dementia.

- That's set to nearly **triple to over 150 million** by 2050 — with the fastest rise in lower- and middle-income countries.

- And these are almost certainly *under*-counts: dementia is often missed or misdiagnosed.

- Worldwide, up to **three in four** people with dementia have never had a diagnosis at all.
:::
...
::: {.column width="45%"}
The new tests are powerful — but they don't replace the clinician. They're one piece of the picture.
...
## The four main types tell themselves apart {.smaller}
...
- **Treatment**: one of the standard tablets often helps a lot — but **avoid antipsychotics**
...
- **Treatment**: above all, **protect the blood vessels** — blood pressure, etc.
:::

## Some "dementias" can be reversed {.smaller}

::: {.callout-warning title="Treatable things that can look like dementia"}
Before anyone concludes it's dementia, these have to be ruled out — because they can be **fixed**:
...
## What a good assessment actually involves {.smaller}
::: {.panel-tabset}
...
### Physical exam
A physical examination, blood tests, and a careful look at current medications — partly to catch the **treatable** causes from a few slides ago.
...
## The first big idea: top up a missing chemical {.smaller}

- In the 1970s, researchers found Alzheimer's brains were short of **acetylcholine** — a chemical messenger the brain uses for memory and attention.
- It was being lost as a particular set of brain cells degenerated.
- The idea that followed: if we can **boost that messenger**, we might improve memory. That gave us the first generation of drugs.
...
- These drugs **slow that breakdown**, so more of the messenger stays around — making the most of what's left.
- That's the key point: they help the *remaining* cells work better. They **don't stop the disease**.
...
:::
::: {.column width="45%"}
- In Alzheimer's, brain cells can be over-stimulated by another messenger (glutamate) — a slow "background noise" that damages them.
...
## So how much do they actually help? {.smaller}
...
## They help some types far more than others {.smaller}
...
- These are often what drives **carer exhaustion**, a move into a care home, and even shorter survival.
...
- **Exercise**
- **Music therapy** — strong evidence for calming agitation
- **The environment**: good light, routine, clear signs, familiar things
- **Supporting the carer** — often the highest-value thing of all
:::
...
- **Especially dangerous in Lewy body dementia** — they can cause severe, lasting harm.
- Only when someone is genuinely at risk — lowest dose, shortest time, reviewed often.
:::
...
- They **don't change where the disease is heading**
- Five years on, the brain has the same disease whether or not someone took them

*That's exactly why the next part matters.*
...
## The big bet: clear the amyloid {.smaller}
...
- So the prediction was simple and bold: **clear the amyloid, and you should slow the disease.**

## It took decades of failure {.smaller}

- Through the 2010s, **drug after drug failed** — solanezumab, bapineuzumab, crenezumab, gantenerumab. Billions spent, no clear benefit.
- Slowly, the field worked out *why*:
...
- The lessons from those failures are what made the recent successes possible.

## Then one drug split the experts {.smaller}

**Aducanumab** (2021) — the controversial one:

- Two big trials **disagreed** with each other
- The US regulator (FDA) approved it anyway — over the near-unanimous objection of its own expert panel
- Europe **refused** it; the **NHS never funded** it
- The maker **withdrew it** in 2024

*It poisoned the well — but it wasn't the end of the story.*

## Lecanemab — the first to clearly work {.smaller}

```{=html}
<div class="trial-meta">~1,800 people · 18 months · early Alzheimer's, confirmed by biomarkers · clears the sticky amyloid clumps</div>
...
## Donanemab — a second drug, same story {.smaller}
...
<div class="trial-grid">
  ...
  <div class="trial-block context">
    ...
    <div class="trial-stat"><span class="trial-num">NHS · rejected</span><span class="trial-lab">NICE, Oct 2024 — same cost-effectiveness verdict</span></div>
...
::: {.incremental}
- **Amyloid must be confirmed** — a scan or spinal-fluid test first
...
- Mostly mild — but a small number are serious, and a few deaths have happened.
- That's why people on these drugs need **repeated MRI scans** to catch problems early.

## One gene drives most of the risk {.smaller}

A common gene, **APOE4**, strongly raises the chance of these scan changes — especially in the ~2% of people who carry **two copies**:
...
- It's a sensitive result: it can carry implications for **family members** too, which has to be handled carefully.
...
- So in the UK they're currently **private-only** — a real **fairness problem**.
- And an open scientific question remains: is clearing amyloid *enough*, or will we need combinations?
:::

## Lots more is coming {.smaller}
...
- And, crucially, **prevention trials** — treating people who carry the risk *before* symptoms ever start

*The first generation of these drugs is just that — the first.*

## Where we stand {.smaller}

::: {.incremental}
- The first disease-modifying drugs **exist** — a genuine milestone. But the benefit is modest, the side-effects real, and UK access restricted.
- A simple **blood test** may soon make diagnosis far easier and earlier.
...
<li>And the single most powerful thing many of us can do is on the next slide.</li>
</ul>
:::
...
## The most powerful thing you can do tonight {background-color="#44475a" .smaller}
...
You don't need a diagnosis — **anyone over 18 can sign up**, with or without memory problems, including carers. You're matched to studies near you, and you choose what to take part in.
...
2. **The treatments we've had for years help a little, but don't stop it.** They're still worth having — and looking after behaviour, mood and the carer matters just as much.

3. **The first disease-modifying drugs have arrived.** A real milestone, but the benefit is modest, the side-effects real, and they're not yet on the NHS.

4. **Prevention may be the biggest prize of all** — and it's something we can all act on, starting now.
:::
